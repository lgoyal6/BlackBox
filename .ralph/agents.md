...
### ws/graph (PHASE-07/08/09)

- **`@langchain/langgraph-checkpoint-mongodb@1.4.0` depends on `mongodb@^6.21.0`; this project pins `mongodb@7.5.0`.** npm nests a second `mongodb@6.21.0` inside the checkpoint package instead of deduping, so the app's `MongoClient` (from the top-level `mongodb`) and the `MongoClient` type `MongoDBSaver`'s constructor expects (from the nested copy) are structurally near-identical but nominally distinct — `tsc` rejects passing one where the other is expected. Not fixable without touching `package.json` (PHASE-01-owned). Fix used in `src/lib/graph/checkpointer.ts`: cast at the one call site — `client as unknown as ConstructorParameters<typeof MongoDBSaver>[0]["client"]` — runtime is unaffected, it's the same object. Any other phase constructing `MongoDBSaver` (or anything from that package) directly will hit the same error.
- **Confirmed `@langchain/langgraph` 1.4.9 export names against the installed `.d.ts`** (resolves the Open Items bullet below): `Annotation`, `Annotation.Root`, `Command`, `START`, `END`, `StateGraph`, `interrupt`, `isInterrupted`, `INTERRUPT` all exist exactly as this repo's specs assumed. `interrupt()` throws `GraphInterrupt` internally; on a checkpointed graph the throw is caught at the graph boundary and `invoke()` resolves normally with the state values plus `result[INTERRUPT]: {id?, value?}[]` — read `result[INTERRUPT][0].value` for the interrupt payload (`isInterrupted()` is the type guard for this). `new Command({ resume: value })` is the documented resume input. `MongoDBSaverParams` is `{ client, dbName?, checkpointCollectionName?, checkpointWritesCollectionName?, enableTimestamps?, ttl? }` — matches contracts.md exactly. **`MongoDBSaver.setup()` returns `Promise<Error[]>`, not `Promise<void>`** — treat a non-empty array as a hard failure (thrown), it's not a warnings list to ignore. Checkpoint documents store `thread_id` as a **top-level field** (confirmed from the compiled saver's own query code), so `checkpointCount` is `col(CHECKPOINTS).countDocuments({ thread_id: incidentId })`.
- **A grep-based acceptance criterion can conflict with a spec's own suggested code.** PHASE-09's spec literally suggests `const { _groundTruth: _, ...safe } = incident;` as the way to strip ground truth in `postmortem.ts`, but the acceptance criterion is `rg -n "_groundTruth" src/lib/memory/postmortem.ts` returning nothing — which that exact line fails, since it names the field as a literal string (even in a destructure key or a comment). Resolution used everywhere in `src/lib/retrieval` and `src/lib/graph` too: never write the literal field name at all: read only the fields you need off the incident object and don't reference the quarantined one, rather than naming-and-discarding it.
- **`tsx -e "<multiline script>"` silently produces no output and exits 0** in this environment (PowerShell/Git-Bash quoting swallows the script) — even a bare relative import produces nothing, no error. Write a real `.ts` file and `npx tsx file.ts` instead of `-e` for anything beyond a one-liner.
- **LangGraph 1.4.9's `StateGraph.addNode()` throws if the node name collides with a state-channel name** — `"X is already being used as a state attribute (a.k.a. a channel), cannot also be used as a node name"`. This only throws inside a real `.compile()` call; it is invisible under `GRAPH_MODE=fake` and invisible to `tsc`. **`IncidentState` has both a `brief` field and a `plan` field, and `GRAPH_NODE_ORDER` uses `"brief"`/`"plan"` as node names** — a real collision in the locked contract. Fixed in `src/lib/graph/compile.ts` by registering those two nodes under different internal LangGraph keys (`brief_step`, `plan_step`) while `wrapNode`'s `name` argument — and therefore `nodeTrail`/emitted `node` events — still use the exact `GraphNode` contract strings. Any other phase that ever adds a `GraphNode` value matching an `IncidentState` field name will hit this too.
- **A live `MongoClient` connection keeps Node's event loop open — a CLI script that never calls `process.exit(0)` on success hangs forever**, even after printing its result and returning from `main()`. This bit `scripts/kill-resume-drill.ts` specifically: the spawned `--resume-only` child printed `RESUME_RESULT` and then hung, so the parent's `child.on("exit")` wait never resolved. Every phase's `tsx`-run scripts that touch `getClient()`/`col()` need an explicit `main().then(() => process.exit(0)).catch(...)` (or equivalent) — don't rely on the process exiting on its own once real Mongo is involved.
- **A checkpointed LangGraph thread does not reset on a fresh `invoke(initialState, config)`** — concat-reducer channels (`timeline`, `nodeTrail`, `retrieved`, `decisionsRecorded`) merge the new input against whatever's already checkpointed for that `thread_id`, they don't replace it. Re-running `graph:local`/`drill` repeatedly against the same `incidentId` accumulates state across runs instead of starting clean. This is exactly why `POST /api/demo/reset` (PHASE-11) deletes checkpoints — it's required between takes on the same incident id, not optional cleanup. Worth a callout in PHASE-15's rehearsal script.
- **`src/lib/memory/index.ts`'s `loadIncident` needs the same Atlas-then-`fixtures/incidents.json` fallback `src/lib/graph/index.ts` has.** Without it, `postmortemNode` (which calls `memory().generateAndWrite()`) throws `"memory: incident ... not found"` the moment `graph:local`/`drill` run against a real cluster before PHASE-04 ingestion exists — this killed an otherwise-passing live `graph:local --auto-confirm` run until fixed.
- **Live-verified against the actual Atlas cluster this session:** `npm run drill` and `npm run graph:local -- --auto-confirm` both pass end-to-end (checkpoints persist, kill-and-resume via a fresh child process works, decision/remediation counts stay identical across the interrupt boundary). `npm run verify:retrieval` correctly stops at its index-readiness gate: `vs_decisions`/`vs_remediations`/`vs_runbooks` are `READY` at 1024 dims, but **`vs_postmortems` does not exist** — this cluster has hit the M0 3-search-index cap called out below and in overview.md. Needs the Flex upgrade (or PHASE-02's documented merge-into-one-`memory`-collection fallback) before real fan-out/relevance probes can run.
...
| 2026-08-13 | ws/graph: `signatureMatch`/`failureMemory` build and run their own `$vectorSearch`/`$unionWith` pipelines directly (via `pipeline.ts`+`fuse.ts`), rather than routing through `index.ts`'s `fanOutFrom` | `fanOutFrom`'s public options only expose `callTypeFamily`; `failureMemory` needs per-source filters (`outcome: "failure"` on remediations, `severityDelta > 0` on postmortems) that don't fit that shape, and routing `signatureMatch`/`failureMemory` through `index.ts` would create a circular import between `index.ts` (which imports both to build the default export) and them. Not a contract change — `RetrievalPort`'s public shape is unaffected. |
| 2026-08-13 | ws/graph: `Hit.meta.callTypeFamily` added to the fan-out projection for `decisions`/`postmortems`/`remediations` (not in contracts.md's illustrative meta table, which is explicitly non-exhaustive — `meta: Record<string, unknown>`) | Needed so the undeclared-filter-path degradation (PHASE-07 spec, pipeline.ts constraint #4) can post-filter by family in TypeScript after retrying without a DB-side filter. |
...
- ~~Confirm the exact `@langchain/langgraph` 1.4.9 export names for `interrupt` and `Command` against the installed types.~~ Done — see ws/graph entry above.
...
- **PHASE-08's `record_decision` graph node writes `actionChosen`/`rationale` as the raw medic utterance** (no LLM split) rather than extracting them separately — `MemoryPort.recordDecision`'s real writer forbids an LLM call internally, and contracts.md §10 implies the actual action/rationale split happens in PHASE-11's `/api/tools/record_decision` route (which receives just `{ incidentId, utterance }`), not in the graph. Whoever builds PHASE-11: decide whether the tool route calls `memory().recordDecision()` directly (making the graph node's own write redundant / a second insert) or whether it drives the graph via `resume()` and lets the graph node be the sole writer — as specced today both paths exist and both call `recordDecision`, which is not obviously wrong (different trigger conditions) but is worth a deliberate call before PHASE-16 wires the live path.
- Everything Atlas-dependent in PHASE-07/08/09 (`npm run verify:retrieval`'s live probes, `npm run drill`, `MongoDBSaver.setup()`, live `fanOut`/`signatureMatch`/`recordDecision` writes) is typechecked and exercised against fakes/fixtures only — nobody has run it against a real cluster yet since no `MONGODB_URI` exists in this environment. First person with a connection string: run those three before trusting them on stage.
