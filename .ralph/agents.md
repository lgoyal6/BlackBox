...
- **Socrata row slices should be fetched sequentially, with one retry on timeout/5xx.** Four parallel `$limit` requests in this environment timed out at 45s; sequential 90s requests completed in ~35s. Empty HTTP 200 bodies are still almost always an encoding bug, not a timeout.
...
| 2026-08-13 | PHASE-02 index script is written for `numDimensions = 1024` (`voyage-3-large`). Live `npm run indexes` has not been run — `.env.local` has no `MONGODB_URI` yet | Copy `.env.example` to `.env.local`, fill a Flex-or-higher URI, then run `EMBEDDINGS_MODE=fake npm run indexes` and wait until all four `vs_*` report READY. Do not start retrieval against BUILDING indexes. |
